...
def mergeAlternately0(word1: str, word2: str) -> str:
    return ''.join(itertools.chain.from_iterable(list(itertools.zip_longest(word1, word2, fillvalue=''))))


def mergeAlternately1(word1: str, word2: str) -> str:
    return ''.join(itertools.chain(*list(itertools.zip_longest(word1, word2, fillvalue=''))))
...
def test():
    ...
    for func in funcs:
        for word1, word2, expected in tab:
            ...
            assert result == expected, f'{func.__qualname__}({word1},{word2}): {result}, {expected=}'
