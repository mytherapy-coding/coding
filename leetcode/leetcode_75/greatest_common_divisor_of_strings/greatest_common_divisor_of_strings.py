...
def gcdOfStrings02(str1: str, str2: str) -> str:
    return next(t for len_t in range(min(len(str1), len(str2)), -1, -1) if
                string_divisor(str1, (t := str1[0:len_t])) and string_divisor(str2, t))
...
def gcdOfStrings04(str1: str, str2: str) -> str:
    res = collections.deque(maxlen=1)
    res.extend(t for i in range(min(len(str1), len(str2)) + 1) if
               string_divisor(str1, (t := str1[:i])) and string_divisor(str2, t))
    return res.pop()


def gcdOfStrings05(str1: str, str2: str) -> str:
    return collections.deque((t for i in range(min(len(str1), len(str2)) + 1) if
                              string_divisor(str1, (t := str1[:i])) and string_divisor(str2, t)), maxlen=1).pop()
...
def test():
    ...
    for gcdOfStrings in gcdOfStrings0, gcdOfStrings01, gcdOfStrings02, gcdOfStrings03, gcdOfStrings04, gcdOfStrings1, gcdOfStrings2, gcdOfStrings3, gcdOfStrings4, gcdOfStrings5:
        for str1, str2, expected in tab:
            result = gcdOfStrings(str1, str2)
            assert result == expected, f'{gcdOfStrings.__qualname__}({str1},{str2}) - {result =} {expected =}'
