...
def expanded_form3(num: int) -> str:
    indexes = range(len(str(num)) - 1, -1, -1)
    res: list[str] = [
        digit + "0" * k for k, digit in zip(indexes, str(num)) if digit != "0"
    ]
...
def expanded_form4(num: int) -> str:
    return " + ".join(
        digit + "0" * k
        for k, digit in zip(range(len(str(num)) - 1, -1, -1), str(num))
        if digit != "0"
    )
...
def tests_expanded_form():
    ...
    for f in funcs:
        for num, expected in tab:
            ...
            assert (
                result == expected
            ), f"test failed on {f.__name__}({num}), {expected=}, {result=}"
